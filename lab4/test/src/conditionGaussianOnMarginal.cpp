        ...
        WHEN("conditionGaussianOnMarginal(muyxjoint, Syxjoint, y, muxcond, Sxcond) is called")
        {
            ...
            THEN("muxcond and Sxcond have correct dimensions")
            {
                ...
                AND_THEN("muxcond matches oracle")
                {
                    ...
                    // TODO
                    CAPTURE_EIGEN(muxcond); // Capture muxcond to be printed on test failure
                    CAPTURE_EIGEN(muxcond_exp); // Capture muxcond_exp to be printed on test failure
                    CHECK(muxcond.isApprox(muxcond_exp));
                }

                AND_THEN("Sxcond is upper triangular")
                {
                    // TODO
                    CAPTURE_EIGEN(Sxcond); // Capture Sxcond to be printed on test failure
                    CHECK(Sxcond.isUpperTriangular());
                }
                ...
                AND_THEN("Sxcond.'*Sxcond matches oracle")
                {
                    ...
                    // TODO
                    CAPTURE_EIGEN(Pxcond); // Capture Pxcond to be printed on test failure
                    CAPTURE_EIGEN(Pxcond_exp); // Capture Sxcond_exp to be printed on test failure
                    CHECK(Pxcond.isApprox(Pxcond_exp));
                }
            }
        }
    }
