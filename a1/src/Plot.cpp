    ...
    , ip()
{
    ...
    double windowHeight       = 540; // Resolution fix!
...
void Plot::render()
{
    ...
    bp.update(pState->cameraPositionDensity(camera).mean(), pState->cameraOrientationEulerDensity(camera).mean());
    fp.update(pState->cameraPositionDensity(camera).mean(), pState->cameraOrientationEulerDensity(camera).mean());
