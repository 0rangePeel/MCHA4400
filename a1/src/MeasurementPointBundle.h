...
Scalar MeasurementPointBundle::logLikelihoodImpl(const Eigen::VectorX<Scalar> & x, const StateSLAM & stateSLAM, const std::vector <std::size_t> & idxLandmarks) const
{
    Eigen::VectorX<Scalar> y = y_.cast<Scalar>();
    Eigen::VectorX<Scalar> h = stateSLAM.predictFeatureBundle<Scalar>(x, camera_, idxLandmarks);
