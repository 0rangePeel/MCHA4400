...
SCENARIO("Camera model")
{
    GIVEN("A camera with no lens distortion")
    {
        ...
        GIVEN("An arbitrary world point and camera pose")
        {
            ...
            Pose pose;
            pose.rCNn = cv::Vec3d(0.792207329559554, 0.959492426392903, 0.655740699156587);
            pose.Rnc = cv::Matx33d
            (
            ...
            REQUIRE(cv::norm(pose.Rnc.t()*pose.Rnc - cv::Matx33d::eye(), cv::NORM_INF) < 100*std::numeric_limits<double>::epsilon()); // Must be orthogonal
